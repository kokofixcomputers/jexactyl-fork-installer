...
if ! fn_exists lib_loaded; then
  ...
  source /tmp/lib.sh || source <(curl -sSL "$GITHUB_BASE_URL/"refs/heads/master/lib/lib.sh)
...
insert_cronjob() {
  ...
  crontab -l | {
    cat
    output "0 0 * * * php /var/www/pterodactyl/artisan p:schedule:renewal >> /dev/null 2>&1"
  } | crontab -
